...
const ClientHome = ({ navigation, route }: any) => {
  const [orders, setOrders] = useState([]);
  const [modalVisible, setModalVisible] = useState(false);
  const [minRange, setMinRange]: any = useState("");
  const [maxRange, setMaxRange]: any = useState("");
  const [storeName, setStoreName]: any = useState("");
  const [returnDate, setReturnDate]: any = useState("");

  useEffect(() => {
    fetch(
      "http://20.70.34.47/api/Client/getAllClientOrders/D6024DF5-411F-48D1-8A47-8D17C05C8D45",
      {
        method: "GET",
      }
    )
  ...
  return (
    <View>
      ...
      >
        ...
        <Button onPress={() => setModalVisible(true)}>Filter Orders</Button>
      </View>
      <ScrollView
      ...
      >
        ...
        >
          ...
          <View style={style.textContainer}>
            <View style={{ width: "auto" }}>
              ...
              >
                ...
                <Text style={{ borderRadius: 10, margin: 5, marginTop: 15 }}>
                  -
                </Text>
          ...
          <Button
            onPress={() =>
              fetch(
                `http://20.70.34.47/api/Client/getOrdersByFilter/D6024DF5-411F-48D1-8A47-8D17C05C8D45?&startRange=${minRange}&endRange=${maxRange}&storeName=${storeName}&date=${returnDate}`,
                {
                  method: "GET",
                }
              )
